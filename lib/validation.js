...
class InputValidator {
  ...
  /**
   * Valida se a string é um UUID (v4).
   * @param {string} uuid - UUID a ser validado.
   * @returns {object} {isValid: boolean, error?: string}
   */
  static validateUUID(uuid) {
    if (!uuid || typeof uuid !== 'string') {
      return { isValid: false, error: 'UUID é obrigatório e deve ser uma string.' };
    }
    // Regex para UUID v4 (formato padrão do Prisma para IDs)
    const uuidV4Regex = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
    if (!uuidV4Regex.test(uuid)) {
      return { isValid: false, error: 'Formato de UUID inválido.' };
    }
    return { isValid: true };
  }
   ...
   * @param {string} content - Conteúdo da mensagem a ser validado.
   * @returns {object} {isValid: boolean, sanitized?: string, error?: string}
   */
  static validateMessageContent(content) {
    if (!content || typeof content !== 'string') {
    ...
    if (sanitizedContent.length < 1) {
      return { isValid: false, error: 'O conteúdo da mensagem não pode estar vazio após a sanitização.' };
    }

    if (sanitizedContent.length > 1000) {
      return { isValid: false, error: 'O conteúdo da mensagem excede o limite de 1000 caracteres.' };
    }

    return { isValid: true, sanitized: sanitizedContent };
  }
}
